...
module.exports = function(input, output, options, callback) {
  ...
  this.sander
    .readdir(input)
    .then(entries => {
      let cpPromises = entries.map(e =>
        this.sander.symlinkOrCopy(input, e).to(output, e)
      );
    ...
    .then(() => {
      ...
      // }
      return this.sander
        .readFile(jsFile, { encoding: "UTF-8" })
        .then(contents => {
          ...
          return installAndActivate(worker).then(() => {
            let absoluteURLsToFetch = options.urls.map(u =>
              url.resolve(options.scope, u)
            );
            let fetchPromises = absoluteURLsToFetch.map(u => {
              ...
              return fetchEvent
                ...
                .then(res => {
                  ...
                  let resolveBackToScope = path.relative(
                    scopePath,
                    parsedURL.path
                  );
                  ...
                  if (
                    resolveBackToScope.lastIndexOf("/") ==
                    resolveBackToScope.length - 1
                  ) {
